...
export default function Home() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    alert(inputValues.email, inputValues.password);
    ...
    .then(response => {
      ...
      window.location.replace("/main/coursesmanagement")
    })
    .catch(error => {
    ...
    <>
      <div className="flex w-screen h-screen overflow-hidden">
        <div className="bg-[#F3F4F5] md:w-[53.9%] h-screen flex flex-col md:px-[115px] px-10 pt-[42px] items-start justify-left text-left">
          ...
          <h1 className="text-[#242424] text-[45px] tracking-[1.0px] leading-[1.3] font-[500] text-left pb-0 ">
            Login to your Account
          </h1>
          <span className="text-[#323A46] text-[13px] font-[400]">
            Enter your Credentials below to get Started with the Kirikeni Okwien
            Admin portal.
          </span>
          <form className="w-full py-6" onSubmit={handleSubmit}>
            <div>
              ...
              <input
                className="w-full border bg-[#FAFAFA] rounded-[8px] py-3 px-4 mb-5 mt-1 "
            ...
            <div className="mb-2">
              <Link href="/forgot-password">
                <span className="text-[#92400E] text-[14px] mb-5">
                  Forgot Password?
                </span>{" "}
              </Link>
            </div>
            <button className="bg-[#005259] rounded-[8px] py-[13px] px-[10px] mt-4 text-[15px] w-full text-white">
              Continue
            </button>
          </form>
        </div>
